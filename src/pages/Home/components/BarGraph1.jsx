...
const BarGraph1 = () => {
  ...
  useEffect(() => {
    ...
    chartContainer.addEventListener('mousewheel', handleMouseWheel, { passive: false });
...
// let remained ;
  const bardata =()=>{
    ...
    console.log(uid,"userid from bargraph")
    axios.post(`https://novapwc.com/api/dietLog?user_id=211&type=food`)
    .then(function(response){
      setBarGraphData(response?.data?.Diet_Log?.[0] )
      setItem(response?.data?.Diet_Log?.[0]?.Log_list )
      console.log(response,"from bargraph-----")
      
      if(response?.data?.Diet_Log?.[0]?.Log_list){
        let consume = response?.data?.Diet_Log?.[0]?.Log_list?.map(obj =>{ return obj.servings_consumed});
        
        setConsumed(consume)
        // console.log(consumed,"arrrrrrrrrrr====");
        let remain = response?.data?.Diet_Log?.[0]?.Log_list?.map(obj =>{ return obj.recommended_servings});
        // console.log(remained,"arrrrrrrrrrr====")
        setRemained(remain);

        let categ = response?.data?.Diet_Log?.[0]?.Log_list?.map(obj =>{ return obj.category});
        setCategory(categ);
...
console.log(barGraphData,"Bargraph data from api")
console.log(item,"servings related data in bargraph--")
 console.log(remained," remained--arrrrrrrrrrr====")
// do in api itself
// const consumed = item?.map(obj =>{ return obj.servings_consumed});
console.log(consumed,"consumed ----arr")
console.log(category,"category names------")

// const remaining =  item?.map(obj =>{ return obj.servings_consumed});
...
// const [date, setDate]=useState("");
  ...
  const options = {
    ...
    // });
  return (
    <div> <PieChart highcharts={Highcharts} options={conf} ref={chartRef} /></div>
  )
}
